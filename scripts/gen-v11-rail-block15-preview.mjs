/**
 * V11-RAIL Fas 3 · Block 15 — … + M Photos (KEEP) viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey/booking/history-logiken stubbas; adapterns foto-mappning
 * (media-filtrering + URL-logik) körs på riktigt från ctx.driveFiles.
 *
 * För raster-PNG: npm run shots:v11rail:block15 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block15-preview.mjs
 */
...
const page =
  ...
  '<title>V11 Rail · Block 15 · M Photos (KEEP) — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 15 — M Photos (KEEP)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → H → I → J → K → L → M → S). M = ' +
  'foton/media ovanpå befintlig drive-files-logik: media-filtrering + URL-logik, varje foto är en native-länk ' +
  '(target=_blank) med data-photo=&lt;id&gt; — befintlig lightbox-handler wire:as om den finns, ingen ny handler. ' +
  'Raster-PNG: <code>npm run shots:v11rail:block15</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block15-M-photos');
