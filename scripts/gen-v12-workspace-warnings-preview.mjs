/**
 * V12 Customer Workspace · Block 3 — browserlös render av Kritiska varningar-modulen
 * (+ av-empty + Journal) i två-zon-shellen i tre iframes (390 / 820 / 1440).
 *
 * För raster-PNG: npm run shots:v12workspace:warnings (kräver chromium).
 * Kör: node ./scripts/gen-v12-workspace-warnings-preview.mjs
 */
...
// Kritiska varningar härleds från automationSignals (fallback när resolvePanelSignals saknas).
const card = Object.assign({}, bcard, {
  ...
  automationSignals: [
    ...
    },
    {
      ...
      why: 'Krävs innan foton tas vid besöket.',
      risk: 'legal',
    },
    {
      ...
      status: 'active',
      what: 'Hög lojalitet',
      why: 'Info-signal (ska EJ visas som kritisk).',
      risk: 'needs_review',
    },
  ],
});
...
const journalEntries = [
  {
    title: 'Konsultation',
    journalType: 'consult',
    note: 'Inledande konsultation. Plan för PRP-behandling upprättad.',
    authorName: 'Dr. Lind',
...
const wsInner = global.window.CcoV12Workspace.render({ card, journalEntries, dossierBundle: null }) || '';
...
const page =
  ...
  '<title>V12 Customer Workspace · Kritiska varningar — viewport-preview</title>' +
  ...
  '<h1>V12 Customer Workspace · Block 3 — Kritiska varningar (sektion 3)</h1>' +
  '<p>Browserlös render bakom <code>?v12workspace=on</code>. Återanvänder V11-adaptern ' +
  'buildCriticalWarnings (blocker/legal-gates; info-signaler exkluderas). Display-only. ' +
  'Raster-PNG: <code>npm run shots:v12workspace:warnings</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v12-workspace-warnings');
