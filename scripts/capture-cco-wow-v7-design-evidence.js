...
function createSyntheticCcoOutput(totalRows = 64) {
  ...
  for (let i = 0; i < totalRows; i += 1) {
    ...
    conversationWorklist.push({
      conversationId: `wow-v7-conv-${i + 1}`,
      messageId: `wow-v7-msg-${i + 1}`,
      mailboxId,
      subject: `WOW v7 tråd ${i + 1}`,
      sender: `kund${i + 1}@example.com`,
      latestInboundPreview: `Förhandsvisning för WOW v7-rad ${i + 1}.`,
...
async function main() {
  ...
  await page.goto(`${baseUrl}/cco?evidence=v7&arcana_reset=1`, {
  ...
  await page.screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-a-bg-panel-levels.png'),
    fullPage: false,
  });
  await page.locator('#ccoInboxControlsColumn').screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-b-left-nav.png'),
  });
  await page.locator('#ccoCenterColumn').screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-c-center-list.png'),
  });
  ...
  await wait(200);
  await page.locator('#ccoCenterColumn').screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-d-selected-row-center.png'),
  });
  if ((await queueRows.count()) > 1) {
    await queueRows.nth(1).hover({ timeout: 12000 });
  } else if ((await queueRows.count()) > 0) {
    await queueRows.first().hover({ timeout: 12000 });
  }
  await wait(160);
  await page.locator('#ccoCenterColumn').screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-e-hover-row-center.png'),
  });
  await page.locator('#ccoReplyColumn').screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-f-right-compose.png'),
  });
  ...
  await wait(220);
  await page.locator('#ccoReplyColumn').screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-g-right-readonly.png'),
  });
  ...
  if ((await indicator.count()) > 0) {
    await indicator.click({ button: 'right' }).catch(() => {});
    await wait(180);
  }
  await page.screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-h-status-rings-menu.png'),
    fullPage: false,
  });
  ...
  await page.screenshot({
    path: path.join(outDir, 'cco-wow-v7-after-i-console-errors.png'),
    fullPage: false,
  });

  await fs.writeFile(
    path.join(outDir, 'cco-wow-v7-after-console-local.json'),
    JSON.stringify(
      {
  ...
  await browser.close();
  console.log('CCO WOW v7 design evidence captured in', outDir);
}
