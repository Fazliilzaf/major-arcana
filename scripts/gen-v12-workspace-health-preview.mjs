/**
 * V12 Customer Workspace · Block 5 — browserlös render av Hälsa-modulen
 * i två-zon-shellen i tre iframes (390 / 820 / 1440).
 *
 * För raster-PNG: npm run shots:v12workspace:health (kräver chromium).
 * Kör: node ./scripts/gen-v12-workspace-health-preview.mjs
 */
...
const bcard = {
  ...
  visits: 7,
  lifetimeValue: 48000,
  // Hälsodeklaration signerad + allergier (testar status + allergi-chips).
  healthDeclaration: { signedAt: '2026-05-10', sourceSystem: 'halso@m365' },
...
const page =
  ...
  '<title>V12 Customer Workspace · Hälsa — viewport-preview</title>' +
  ...
  '<h1>V12 Customer Workspace · Block 5 — Hälsa (sektion 4)</h1>' +
  '<p>Browserlös render bakom <code>?v12workspace=on</code>. Återanvänder V11-adaptern ' +
  'buildHealthPreview + befintlig jump (<code>data-kk-jump="kk-card-halsa"</code>). ' +
  'Läkemedel/kontraindikationer deferreras (visas när data finns). ' +
  'Raster-PNG: <code>npm run shots:v12workspace:health</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v12-workspace-health');
