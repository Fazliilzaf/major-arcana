/**
 * V12 Customer Workspace · Block 12 — browserlös render av Kommunikation-modulen
 * i två-zon-shellen i tre iframes (390 / 820 / 1440).
 *
 * För raster-PNG: npm run shots:v12workspace:communication (kräver chromium).
 * Kör: node ./scripts/gen-v12-workspace-communication-preview.mjs
 */
...
const occasionTimeline = [
  {
    ...
    detail: { subject: 'Din tid 1 juli' },
    occurredAt: '2026-06-20',
  },
  { kind: 'sms', title: 'Påminnelse-SMS', summary: '2026-06-19' },
  { kind: 'call', title: 'Uppföljningssamtal', summary: '2026-06-15' },
];

const railCtx = { bcard, card, journalEntries, occasionTimeline, dossierBundle: null };
const railInner = global.window.CcoV11Rail.render(railCtx) || '';
const wsInner =
  global.window.CcoV12Workspace.render({
    card,
    bcard,
    journalEntries,
    occasionTimeline,
...
const page =
  ...
  '<title>V12 Customer Workspace · Kommunikation — viewport-preview</title>' +
  ...
  '<h1>V12 Customer Workspace · Block 12 — Kommunikation (sektion 10)</h1>' +
  '<p>Browserlös render bakom <code>?v12workspace=on</code>. Återanvänder V11-adaptern ' +
  'buildCommunicationFromState (occasionTimeline + fallback) + befintlig svarstudio ' +
  '(<code>data-v9-quick="reply"</code>), ingen ny handler. Avsändare/snippet/läst-status ' +
  'saknas i källan → uppskjutet. Tom lista → explicit empty-state. ' +
  'Raster-PNG: <code>npm run shots:v12workspace:communication</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v12-workspace-communication');
