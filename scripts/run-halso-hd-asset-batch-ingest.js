...
/**
 * Phase 1 batch-ingest: m365_halso PDF → parse → match → prod PUT.
 *
 * Usage:
...
function parseArgs(argv) {
  const args = {
    batch: 1,
    batchSize: Number(process.env.HALSO_HD_PHASE1_BATCH_SIZE || '50') || 50,
    index: DEFAULT_INDEX,
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (token === '--batch') args.batch = Number(argv[++i]) || 1;
      ...
      console.log(
        'Usage: node scripts/run-halso-hd-asset-batch-ingest.js --batch 1 [--batch-size 50] [--dry-run|--commit]'
      );
      process.exit(0);
    }
  }
...
async function main() {
  ...
  console.error(`Prod: ${BASE} · catalog=${catalogEntries.length} PDF`);
