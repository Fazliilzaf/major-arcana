/**
 * V11-RAIL Fas 3 · Block 18 — … + P Communication (KEEP) viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey/booking/history-logiken stubbas; adapterns kommunikations-mappning
 * (occasionTimeline → mail/sms/call) körs på riktigt.
 *
 * För raster-PNG: npm run shots:v11rail:block18 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block18-preview.mjs
 */
...
const card = Object.assign({}, bcard, {
  ...
  importantNote: 'Föredrar förmiddagstider.',
  upcomingBookings: [
    {
...
const occasionTimeline = [
  ...
  },
  {
    ...
    detail: { subject: 'PRP-hår · 3 sessioner' },
...
const dossierBundle = {
  ...
  recentEvents: [{ kind: 'note', subject: 'Kund ringde om ombokning.', at: '2026-06-18' }],
};
...
const page =
  ...
  '<title>V11 Rail · Block 18 · P Communication (KEEP) — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 18 — P Communication (KEEP)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → H → I → J → K → L → M → N → O → P → S). P = ' +
  'kommunikationslogg (mejl/SMS/samtal) ovanpå befintlig dossier-logik: typ-ikon + text + meta, display-only; ' +
  '"Svarstudio" bär data-v9-quick=&quot;reply&quot; (befintlig handler), ingen ny. ' +
  'Raster-PNG: <code>npm run shots:v11rail:block18</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block18-P-communication');
