/**
 * V12 Customer Workspace · Block 13 — browserlös render av Ekonomi-modulen
 * i två-zon-shellen i tre iframes (390 / 820 / 1440).
 *
 * För raster-PNG: npm run shots:v12workspace:economy (kräver chromium).
 * Kör: node ./scripts/gen-v12-workspace-economy-preview.mjs
 */
...
const page =
  ...
  '<title>V12 Customer Workspace · Ekonomi — viewport-preview</title>' +
  ...
  '<h1>V12 Customer Workspace · Block 13 — Ekonomi (sektion 11)</h1>' +
  '<p>Browserlös render bakom <code>?v12workspace=on</code>. Återanvänder V11-adaptern ' +
  'buildEconomyFromCard (parity.buildEconomyFields + fallback). Display-only, ingen handler. ' +
  'Fakturor/betalstatus, skuld-breakdown och rabatter saknas i källan → uppskjutet. ' +
  'Ingen ekonomidata → explicit empty-state. ' +
  'Raster-PNG: <code>npm run shots:v12workspace:economy</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v12-workspace-economy');
