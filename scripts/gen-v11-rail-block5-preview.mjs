/**
 * V11-RAIL Fas 3 · Block 5 — D + A + V + B + C viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440) — varje
 * iframe har egen viewport → @media (canon §4) resolvar per bredd.
 *
 * För raster-PNG: npm run shots:v11rail:block5 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block5-preview.mjs
 */
...
const signals = [
  {
    ruleId: 'customer.missing_health_declaration',
    status: 'active',
  ...
  {
    ruleId: 'customer.cooling_off_active',
    status: 'active',
...
const page =
  ...
  '<title>V11 Rail · Block 5 · D + A + V + B + C — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 5 — D Critical warnings + A + V + B + C</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C). Röda top-banners överst; ' +
  'betänketid (info) exkluderas. Varje iframe har egen viewport så @media (canon §4) resolvar per bredd. ' +
  'Raster-PNG: <code>npm run shots:v11rail:block5</code> i miljö med browser.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';
