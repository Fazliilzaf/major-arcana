...
STATS="$(curl -fsS "$BASE/api/v1/cco-patient-master/stats")"
echo "$STATS" | grep -q '"totalPatients":5' || fail "förväntar 5 pilotkunder"
pass "pilotkunder i patient-master"

PATIENT="$(curl -fsS "$BASE/api/v1/cco-patient-master/patient?patientId=$PATIENT_ID")"
echo "$PATIENT" | grep -q '"displayName"' || fail "patient lookup"
pass "patient lookup"

AGREEMENT="$(curl -fsS "$BASE/api/v1/cco-treatment-agreement/patient-agreement?patientId=$PATIENT_ID")"
echo "$AGREEMENT" | grep -q '"agreement"' || fail "agreement API"
pass "avtal-API"

GATE="$(curl -fsS "$BASE/api/v1/cco-treatment-agreement/booking-gate?patientId=$PATIENT_ID&serviceId=fue")"
...
JOURNAL="$(curl -fsS "$BASE/api/v1/cco-journal/entries?patientId=$PATIENT_ID")"
