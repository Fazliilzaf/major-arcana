...
function createSyntheticCcoOutput(totalRows = 56) {
  const now = Date.now();
  const mailboxPool = [
  ...
  for (let i = 0; i < totalRows; i += 1) {
    ...
    const priorityLevel = i < 5 ? 'Critical' : i < 16 ? 'High' : i < 34 ? 'Medium' : 'Low';
    ...
    conversationWorklist.push({
      conversationId: `v6-conv-${i + 1}`,
      messageId: `v6-msg-${i + 1}`,
      mailboxId,
      subject: `CCO v6 tråd ${i + 1}`,
      sender: `kund${i + 1}@example.com`,
      latestInboundPreview: `Förhandsvisning för CCO v6 rad ${i + 1}.`,
...
async function main() {
  ...
  const output = createSyntheticCcoOutput(56);
  ...
  await page.goto(`${baseUrl}/cco?evidence=v6`, { waitUntil: 'networkidle', timeout: 45000 });
  ...
  await page.screenshot({
    path: path.join(outDir, 'cco-v6-1-start-3col-noscroll.png'),
    fullPage: false,
  });
  ...
  await page.locator('#ccoCenterColumn').screenshot({
    path: path.join(outDir, 'cco-v6-2-center-10plus-rows.png'),
  });

  const feedRows = page.locator('#ccoInboxFeedList .ccoFeedSelectBtn');
  const feedCount = await feedRows.count();
  if (feedCount > 5) {
    await feedRows.nth(5).click({ timeout: 12000 });
    await wait(220);
  } else if (feedCount > 0) {
    await feedRows.first().click({ timeout: 12000 });
    await wait(220);
  }
  await page.locator('#ccoCenterColumn').screenshot({
    path: path.join(outDir, 'cco-v6-3-selected-row-state.png'),
  });
  ...
  if (rowCount > 1) {
    await rows.nth(1).click({ timeout: 12000 });
    await wait(220);
  } else if (rowCount > 0) {
  ...
  await page.evaluate(() => {
    const replyCol = document.getElementById('ccoReplyColumn');
    const mainBlocks = document.getElementById('ccoReplyMainBlocks');
    const compose = document.getElementById('ccoComposeStudio');
    const conversation = document.getElementById('ccoConversationColumn');
    const editorBlock = document.querySelector('#ccoComposeStudio .cco-editor-block');
    const editor = document.getElementById('ccoDraftBodyInput');

    if (replyCol) {
      replyCol.style.height = 'auto';
      replyCol.style.maxHeight = 'none';
      replyCol.style.overflow = 'visible';
    }
    if (mainBlocks) {
      mainBlocks.style.display = 'grid';
      mainBlocks.style.gridTemplateRows = 'auto auto';
      mainBlocks.style.gap = '10px';
      mainBlocks.style.height = 'auto';
      mainBlocks.style.maxHeight = 'none';
    }
    if (conversation) {
      conversation.style.minHeight = '120px';
    }
    if (compose) {
      compose.style.overflow = 'visible';
      compose.style.height = 'auto';
      compose.style.maxHeight = 'none';
      compose.style.gap = '8px';
      Array.from(compose.children).forEach((node, index) => {
        if (node && node.nodeType === Node.ELEMENT_NODE) {
          node.setAttribute('data-v6-block-label', `Block ${index + 1}`);
          node.querySelectorAll('.cco-v6-block-badge').forEach((badge) => badge.remove());
          const badge = document.createElement('div');
          badge.className = 'cco-v6-block-badge';
    ...
    if (editorBlock) {
      editorBlock.style.minHeight = '0';
      editorBlock.style.gap = '6px';
    }
    if (editor) {
      editor.style.minHeight = '88px';
      editor.style.height = '88px';
      editor.style.maxHeight = '88px';
    }

    let styleEl = document.getElementById('ccoV6EvidenceLabels');
    if (!styleEl) {
      styleEl = document.createElement('style');
      styleEl.id = 'ccoV6EvidenceLabels';
      styleEl.textContent = `
        #ccoComposeStudio > .cco-reply-block::before {
          content: attr(data-v6-block-label);
          display: none;
        }
        .cco-v6-block-badge {
          display: inline-flex;
  ...
  await page.locator('#ccoComposeStudio').screenshot({
    path: path.join(outDir, 'cco-v6-4-compose-4-blocks.png'),
  });
  ...
  await page.locator('#ccoReplyColumn').screenshot({
    path: path.join(outDir, 'cco-v6-5-readonly-empty-state.png'),
  });

  const consoleErrors = consoleMessages.filter((entry) => entry.type === 'error');
  await fs.writeFile(
    path.join(outDir, 'cco-v6-console-local.json'),
    JSON.stringify(
      {
  ...
  await browser.close();
  console.log('CCO v6 evidence captured in', outDir);
}
