/**
 * V11-RAIL Fas 3 · Block 2 — A Profile + B Smart information viewport-preview
 * (browserlös). Renderar de RIKTIGA komponenterna och bäddar in dem i tre
 * iframes (390 / 820 / 1440) — varje iframe har egen viewport → @media
 * (canon §4) resolvar per bredd. Öppna resultatet i valfri webbläsare.
 *
 * För raster-PNG: npm run shots:v11rail:block2 (kräver chromium).
 *
 * Kör: node ./scripts/gen-v11-rail-block2-preview.mjs
 */
  ...
  automationSignals: [
    {
      ...
      risk: 'review',
      priority: 90,
      what: 'Skicka offert för signering',
      why: 'Offert skapad efter konsultation, väntar på utskick.',
      next: 'Granska utkast och skicka för e-signering.',
      humanApprovalRequired: true,
      confidence: 'hög',
    },
    { status: 'active', risk: 'info', priority: 40, what: 'Boka uppföljning' },
  ],
};
const inner = global.window.CcoV11Rail.render({ bcard, card });
...
const page =
  ...
  '<title>V11 Rail · Block 2 · A Profile + B Smart information — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 2 — A Profile + B Smart information</h1>' +
  '<p>Riktiga komponenterna (cco-v11-rail.css + logik-lager + adapter + renderer) i tre iframes. ' +
  'Varje iframe har egen viewport-bredd så @media (canon §4) resolvar per bredd. ' +
  'Raster-PNG genereras med <code>npm run shots:v11rail:block2</code> i miljö med browser.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block2-B-smart-info');
