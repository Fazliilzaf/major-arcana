/**
 * V11-RAIL Fas 3 · Block 10 — … + H Bookings (KEEP) viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey- och booking-logiken stubbas deterministiskt; adapterns mappning och
 * H:s bokningslista (kommande tider, status, personal) körs på riktigt.
 *
 * För raster-PNG: npm run shots:v11rail:block10 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block10-preview.mjs
 */
...
const page =
  ...
  '<title>V11 Rail · Block 10 · H Bookings (KEEP) — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 10 — H Bookings (KEEP)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → H → S). H = kommande ' +
  'bokningar ovanpå befintlig bokningslogik: varje rad bär data-v9-section-link=&quot;upcoming&quot; (öppnar ' +
  'tidslinjen där avboka/omboka lever) och sektionen har data-v9-quick=&quot;confirm&quot; — befintliga handlers, ' +
  'inga nya, bokningsworkflow bevaras. Raster-PNG: <code>npm run shots:v11rail:block10</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block10-H-bookings');
