/**
 * V11-RAIL Fas 3 · Block 9 — D + A + V + B + C + E + F + G + S viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey- och booking-logiken stubbas deterministiskt; adapterns mappning,
 * slugs och sticky-footer-data (N kommande tider, ready-state) körs på riktigt.
 *
 * För raster-PNG: npm run shots:v11rail:block9 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block9-preview.mjs
 */
...
// Deterministiska stubbar (kanoniska 9 steg + booking-lagret) för förhandsvisningen.
...
const page =
  ...
  '<title>V11 Rail · Block 9 · D + A + V + B + C + E + F + G + S — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 9 — S Sticky Footer (persistent åtgärdsrad)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → S). S = persistent ' +
  'åtgärdsrad: "Boka nästa" (återanvänder ord48-kalenderhandlern, aktiv när kunden är redo) + "Bekräfta ' +
  'kommande tider (N)" där N kommer från riktig booking-data. Sticky längst ner på mobil/tablet, rail-lokal ' +
  'på desktop. Varje iframe har egen viewport så @media (canon §4) resolvar per bredd. ' +
  'Raster-PNG: <code>npm run shots:v11rail:block9</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block9-S-sticky');
