/**
 * V12 Customer Workspace · Block 10 — browserlös render av Dokument-modulen
 * i två-zon-shellen i tre iframes (390 / 820 / 1440).
 *
 * För raster-PNG: npm run shots:v12workspace:documents (kräver chromium).
 * Kör: node ./scripts/gen-v12-workspace-documents-preview.mjs
 */
...
global.window.CcoKundkortKkx = {
  buildCanonicalJourneyLive: function () {
    return { activeStep: 4, nextLabel: 'Offert + behandlingsplan', steps: [] };
  },
...
// Dokument-payload (samma kontrakt som legacy resolveV11DocumentPayload).
global.window.CcoV9CustomersParity = {
  resolveV11DocumentPayload: function () {
    return {
      offers: [
        {
          title: 'Behandlingsplan PRP-hår',
          amount: '18 000 kr',
          status: 'sent',
          registryId: 'offert_prp',
          journeyStep: '4',
        },
        {
          title: 'Offert Botox',
      ...
      autoDocs: [
        { title: 'Samtycke PRP', status: 'pending', registryId: 'auto_samtycke_prp' },
        { title: 'Eftervårdsinstruktion', status: 'planned', registryId: 'auto_eftervard' },
      ],
    };
  },
};
...
const driveFiles = [
  { id: 'f-1', originalFileName: 'Journal-2026-06-18.pdf', fileType: 'journal_pdf' },
  { id: 'f-2', originalFileName: 'Remiss.docx' },
];

const railCtx = { bcard, card, journalEntries, driveFiles, dossierBundle: null };
const railInner = global.window.CcoV11Rail.render(railCtx) || '';
const wsInner =
  global.window.CcoV12Workspace.render({ card, bcard, journalEntries, driveFiles, dossierBundle: null }) ||
  '';
...
const page =
  ...
  '<title>V12 Customer Workspace · Dokument — viewport-preview</title>' +
  ...
  '<h1>V12 Customer Workspace · Block 10 — Dokument (sektion 9)</h1>' +
  '<p>Browserlös render bakom <code>?v12workspace=on</code>. Återanvänder V11-adaptrarna ' +
  'buildOffersFromPayload + buildAutoDocsFromPayload + buildFilesFromDriveFiles + befintliga ' +
  '<code>data-v11-doc-*</code>-attribut (dokument-preview, ingen ny handler; filer = native länkar). ' +
  'Tomma listor → explicit empty-state. ' +
  'Raster-PNG: <code>npm run shots:v12workspace:documents</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v12-workspace-documents');
