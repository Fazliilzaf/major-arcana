...
if [[ "$RUN_PREFLIGHT" == "true" ]]; then
  ...
  set +e
  BASE_URL="$BASE_URL" ARCANA_OWNER_EMAIL="$EMAIL" ARCANA_OWNER_PASSWORD="$PASSWORD" ARCANA_DEFAULT_TENANT="$TENANT_ID" ARCANA_OWNER_MFA_RECOVERY_CODE="$MFA_RECOVERY_CODE" ARCANA_PREFLIGHT_FORCE_OPS_ON_GUARD_FAIL="$FORCE_GUARD_CONTINUE" npm run "$PREFLIGHT_SCRIPT" -- "${PREFLIGHT_ARGS[@]}"
  PREFLIGHT_EXIT_CODE=$?
  set -e
...
BASE_URL="$BASE_URL" ARCANA_OWNER_EMAIL="$EMAIL" ARCANA_OWNER_PASSWORD="$PASSWORD" ARCANA_DEFAULT_TENANT="$TENANT_ID" ARCANA_OWNER_MFA_RECOVERY_CODE="$MFA_RECOVERY_CODE" npm run smoke:public

if [[ "$RUN_MAIL_SEEDS" == "true" ]]; then
  echo "3) Mail seeds apply+activate"
  BASE_URL="$BASE_URL" ARCANA_OWNER_EMAIL="$EMAIL" ARCANA_OWNER_PASSWORD="$PASSWORD" ARCANA_DEFAULT_TENANT="$TENANT_ID" ARCANA_OWNER_MFA_RECOVERY_CODE="$MFA_RECOVERY_CODE" npm run mail:seeds:apply-activate
  echo
else
...
echo "4) Pilot report"
BASE_URL="$BASE_URL" ARCANA_OWNER_EMAIL="$EMAIL" ARCANA_OWNER_PASSWORD="$PASSWORD" ARCANA_DEFAULT_TENANT="$TENANT_ID" ARCANA_OWNER_MFA_RECOVERY_CODE="$MFA_RECOVERY_CODE" npm run report:pilot
