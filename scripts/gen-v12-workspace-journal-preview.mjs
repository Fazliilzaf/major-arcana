/**
 * V12 Customer Workspace · Block 0 — browserlös helhets-render av Journal-modulen
 * i två-zon-shellen (Zon 1 = V11-rail, Zon 2 = V12-arbetsyta) i tre iframes
 * (390 / 820 / 1440). Beslutsunderlag / screenshot-referens.
 *
 * För raster-PNG: npm run shots:v12workspace:journal (kräver chromium).
 * Kör: node ./scripts/gen-v12-workspace-journal-preview.mjs
 */
...
const card = Object.assign({}, bcard, {
  ...
  upcomingBookings: [
    { iso: '2026-06-25T09:30:00Z', title: 'PRP-behandling', sub: 'Hårbotten · Fazli', state: 'ready', stateLabel: '✓ Redo' },
  ],
  bookingHistory: [
    { iso: '2026-05-12T10:00:00Z', title: 'PRP-behandling', sub: 'Hårbotten · Fazli', state: 'done', stateLabel: 'Klar' },
  ],
});

// Journal — full djupvy: full nottext (workspace), inte railens snippet.
const journalEntries = [
  {
    title: 'PRP-behandling · session 3',
    journalType: 'tp_treatment',
    clinicalSummary:
      'Tredje PRP-sessionen i hårbotten. 6 ml PRP framställt enligt protokoll, injicerat i ' +
      'frontotemporal region. Patienten tolererade behandlingen väl, ingen vasovagal reaktion. ' +
      'Lätt rodnad postop, förväntad. Nästa session inplaneras om 4 veckor.',
    authorName: 'Dr. Lind',
  ...
  },
  {
    ...
    journalType: 'follow',
    note:
      'Patienten rapporterar begynnande förbättring av hårtäthet i frontlinjen. Foton tagna för ' +
      'jämförelse. Utkast — kompletteras efter nästa besök.',
    authorName: 'Egzona',
...
const wsInner = global.window.CcoV12Workspace.render({ journalEntries }) || '';
...
const page =
  ...
  '<title>V12 Customer Workspace · Journal — viewport-preview</title>' +
  ...
  '<h1>V12 Customer Workspace · Block 0 — Journal-modul (Zon 1 rail + Zon 2 arbetsyta)</h1>' +
  '<p>Browserlös render av de riktiga komponenterna bakom <code>?v12workspace=on</code>. ' +
  'Mobil: V12 äger ytan (Zon 1 dold). iPad/webb: Zon 1-rail bredvid Zon 2. ' +
  'Raster-PNG: <code>npm run shots:v12workspace:journal</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';
