...
const mailboxEmail = (process.env.ARCANA_MAILBOX || 'halso@hairtpclinic.com').toLowerCase();
...
async function main() {
  ...
  console.log(`== Hälsodeklaration ingest (${mailboxEmail}) ==`);
  console.log('sync…');
  ...
  console.log(
    JSON.stringify(
      {
        saved: sync.result?.ingestResult?.totalSaved,
        fetched: sync.result?.ingestResult?.totalFetched,
      },
      null,
      2
    )
