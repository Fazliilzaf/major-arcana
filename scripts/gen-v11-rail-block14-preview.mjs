/**
 * V11-RAIL Fas 3 · Block 14 — … + L Auto-documents (KEEP) viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey/booking/history-logiken stubbas; adapterns auto-dokument-mappning
 * (titel/status/steg/previewable) körs på riktigt från dossierBundle.documents.autoDocs.
 *
 * För raster-PNG: npm run shots:v11rail:block14 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block14-preview.mjs
 */
...
const inner = global.window.CcoV11Rail.render({ bcard, card, dossierBundle, journalEntries });
...
const page =
  ...
  '<title>V11 Rail · Block 14 · L Auto-documents (KEEP) — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 14 — L Auto-documents (KEEP)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → H → I → J → K → L → S). L = ' +
  'auto-genererade dokument/mallar ovanpå befintlig dokument-logik: titel/status/steg, varje rad bär ' +
  'data-v11-doc-row/data-v11-doc-registry/data-v11-doc-previewable + data-v11-doc-filler=&quot;auto&quot; (öppnar ' +
  'mall-/dokument-preview) — befintliga handlers, inga nya. Raster-PNG: <code>npm run shots:v11rail:block14</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block14-L-autodocs');
