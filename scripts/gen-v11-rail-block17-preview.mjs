/**
 * V11-RAIL Fas 3 · Block 17 — … + O Notes (KEEP) viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey/booking/history-logiken stubbas; adapterns notes-mappning
 * (importantNote + allergier + recentEvents) körs på riktigt.
 *
 * För raster-PNG: npm run shots:v11rail:block17 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block17-preview.mjs
 */
...
const card = Object.assign({}, bcard, {
  ...
  importantNote: 'Föredrar förmiddagstider. Ring inte före kl 10. Önskar samma behandlare.',
  upcomingBookings: [
    {
...
const driveFiles = [{ id: 'd1', fileType: 'journal_pdf', originalFileName: 'Journal.pdf' }];
const dossierBundle = {
  activeVisit: {
    visible: true,
    state: 'in_progress',
  ...
  recentEvents: [
    {
      kind: 'note',
      subject: 'Kund ringde om ombokning — löst, ny tid 25 juni.',
      at: '2026-06-18',
    },
    {
      kind: 'anteckning',
      body: 'Diskuterade förväntningar kring PRP-resultat, realistisk tidslinje 3-6 mån.',
      at: '2026-04-02',
    },
  ],
};
...
const page =
  ...
  '<title>V11 Rail · Block 17 · O Notes (KEEP) — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 17 — O Notes (KEEP)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → H → I → J → K → L → M → N → O → S). O = ' +
  'anteckningar/noteringar (importantNote + allergier + recentEvents) ovanpå befintlig dossier-logik: viktig/medicinsk/' +
  'notering-ton, display-only som legacy, ingen handler. Raster-PNG: <code>npm run shots:v11rail:block17</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block17-O-notes');
