/**
 * V11-RAIL Fas 3 · Block 20 — … + R Insights (LATER) viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey/booking-logiken stubbas; adapterns insikts-mappning (sortSignals →
 * insiktskort) körs på riktigt från card.automationSignals.
 *
 * För raster-PNG: npm run shots:v11rail:block20 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block20-preview.mjs
 */
...
const card = Object.assign({}, bcard, {
  ...
  automationSignals: [
    ...
    {
      ruleId: 'customer.next_best_action',
      status: 'active',
      what: 'Boka uppföljning',
      next: 'Bekräfta kommande tid och förbered formulär.',
      risk: '',
    },
  ],
});
...
const page =
  ...
  '<title>V11 Rail · Block 20 · R Insights (LATER) — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 20 — R Insights (LATER)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → H → I → J → K → L → M → N → O → P → Q → R → S). R = ' +
  'operativa insikter byggda på kundens riktiga automation-signaler (via CcoKunderSmartNextStep.sortSignals): ' +
  'ton blocker/review/insight, display-only, ingen handler. Inga aktiva signaler → explicit empty-state. ' +
  'Raster-PNG: <code>npm run shots:v11rail:block20</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block20-R-insights');
