/**
 * V11-RAIL Fas 3 · Block 16 — … + N Files (KEEP) viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey/booking/history-logiken stubbas; adapterns fil-mappning (dokument-
 * filtrering + URL-logik + badge) körs på riktigt från ctx.driveFiles.
 *
 * För raster-PNG: npm run shots:v11rail:block16 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block16-preview.mjs
 */
...
const driveFiles = [
  ...
  { id: 'd4', originalFileName: 'Kostnadsspecifikation.xlsx' },
  { id: 'd5', originalFileName: 'Samtyckesblankett.pdf' },
];
const dossierBundle = {
  activeVisit: {
    visible: true,
...
const page =
  ...
  '<title>V11 Rail · Block 16 · N Files (KEEP) — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 16 — N Files (KEEP)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → H → I → J → K → L → M → N → S). N = ' +
  'dokumentfiler (icke-media) ovanpå befintlig drive-files-logik: ikon + filnamn + badge (PDF/CCO/ext), varje fil är ' +
  'en native-länk (target=_blank) — samma sätt som legacy dokumentrader, ingen ny handler. ' +
  'Raster-PNG: <code>npm run shots:v11rail:block16</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block16-N-files');
