/**
 * V11-RAIL Fas 3 · Block 13 — … + K Offers (KEEP) viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna i tre iframes (390 / 820 / 1440).
 * Journey/booking/history-logiken stubbas; adapterns offert-mappning
 * (titel/belopp/status/steg) körs på riktigt från dossierBundle.documents.offers.
 *
 * För raster-PNG: npm run shots:v11rail:block13 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block13-preview.mjs
 */
...
const dossierBundle = {
  ...
  documents: {
    offers: [
      ...
      {
        registryId: 'offert_followup',
        title: 'Behandlingsplan · uppföljning',
        amount: '',
        status: 'planned',
        journeyStep: 7,
      },
...
const page =
  ...
  '<title>V11 Rail · Block 13 · K Offers (KEEP) — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 13 — K Offers (KEEP)</h1>' +
  '<p>Riktiga komponenterna i tre iframes (ordning D → A → V → B → C → E → F → G → H → I → J → K → S). K = ' +
  'offerter/behandlingsplaner ovanpå befintlig dokument-logik: titel/belopp/status/steg, varje rad bär ' +
  'data-v11-doc-row/data-v11-doc-registry/data-v11-doc-previewable (öppnar offert-preview/dokumentvyn) — ' +
  'befintliga handlers, inga nya. Raster-PNG: <code>npm run shots:v11rail:block13</code>.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block13-K-offers');
