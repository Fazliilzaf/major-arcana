/**
 * V11-RAIL Fas 3 · Block 3 — A + B + C viewport-preview (browserlös).
 * Renderar de RIKTIGA komponenterna och bäddar in dem i tre iframes
 * (390 / 820 / 1440) — varje iframe har egen viewport → @media (canon §4)
 * resolvar per bredd. Öppna resultatet i valfri webbläsare.
 *
 * För raster-PNG: npm run shots:v11rail:block3 (kräver chromium).
 * Kör: node ./scripts/gen-v11-rail-block3-preview.mjs
 */
...
const page =
  ...
  '<title>V11 Rail · Block 3 · A + B + C — viewport-preview</title>' +
  ...
  '<h1>V11 Rail · Fas 3 · Block 3 — A Profile + B Smart information + C Stats</h1>' +
  '<p>Riktiga komponenterna i tre iframes. Varje iframe har egen viewport-bredd så @media ' +
  '(canon §4) resolvar per bredd. Raster-PNG: <code>npm run shots:v11rail:block3</code> i miljö med browser.</p>' +
  '<div class="row">' +
  frames +
  '</div></body></html>';

const outDir = join(root, 'docs/handover/MOCKUPS/v11-rail-block3-C-stats');
