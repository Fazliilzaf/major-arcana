...
if (!P || typeof P.parse !== 'function') {
  console.error('Parsern är inte exponerad korrekt — kolla att IIFE:n registrerar window.MajorArcanaSystemMailParser');
  process.exit(1);
}
...
describe('Phase 1: subject-parsing per system', () => {
  it('Cliento "Ny bokning: <Namn>"', () => {
    const r = P.parse({
      senderEmail: 'no-reply@cliento.com', senderName: 'No Reply',
      subject: 'Ny bokning: Anna Karlsson - 15 maj 14:00', body: '',
    });
  ...
  it('Cliento "Bokning · Namn · ..."', () => {
    const r = P.parse({
      senderEmail: 'no-reply@cliento.com', senderName: 'No Reply',
      subject: 'Bokning · Anna Karlsson · 15 maj 14:00', body: '',
    });
    eq(r, { customerName: 'Anna Karlsson', systemLabel: 'via Cliento' });
  });
  it('Cliento "Namn har bokat"', () => {
    const r = P.parse({
      senderEmail: 'no-reply@cliento.com', senderName: 'No Reply',
      subject: 'Anna Karlsson har bokat tid', body: '',
    });
    eq(r, { customerName: 'Anna Karlsson', systemLabel: 'via Cliento' });
  });
  it('Smartdocs "Namn har signerat"', () => {
    const r = P.parse({
      senderEmail: 'notifications@smartdocs.io', senderName: 'Smartdocs',
      subject: 'Andreas Monasterio har signerat dokumentet', body: '',
    });
    eq(r, { customerName: 'Andreas Monasterio', systemLabel: 'via Smartdocs' });
  });
  it('Bokadirekt "Booking Request from <Namn>"', () => {
    const r = P.parse({
      senderEmail: 'transactional@bokadirekt.se', senderName: 'Bokadirekt',
      subject: 'Booking Request from Jimmy Skoglund', body: '',
    });
    eq(r, { customerName: 'Jimmy Skoglund', systemLabel: 'via Bokadirekt' });
  });
  it('GetAccept "Namn har öppnat dokumentet"', () => {
    const r = P.parse({
      senderEmail: 'no-reply@getaccept.com', senderName: 'GetAccept',
      subject: 'Albert Mattsson har öppnat dokumentet', body: '',
    });
    eq(r, { customerName: 'Albert Mattsson', systemLabel: 'via GetAccept' });
  });
  it('Google Calendar "Accepterat: <Namn>"', () => {
    const r = P.parse({
      senderEmail: 'calendar-notification@google.com', senderName: 'Google Calendar',
      subject: 'Accepterat: Anna Karlsson | Klinikbesök', body: '',
    });
    eq(r, { customerName: 'Anna Karlsson', systemLabel: 'via Kalender' });
  });
  it('Kivra "från <Namn>"', () => {
    const r = P.parse({
      senderEmail: 'no-reply@kivra.com', senderName: 'Kivra',
      subject: 'Du har fått ett dokument från Anna Karlsson', body: '',
    });
    eq(r, { customerName: 'Anna Karlsson', systemLabel: 'via Kivra' });
  });
  it('Pipedrive "Activity assigned: <Namn>"', () => {
    const r = P.parse({
      senderEmail: 'notifications@pipedrive.com', senderName: 'Pipedrive',
      subject: 'Activity assigned: Anna Karlsson - Follow up', body: '',
    });
...
describe('Phase 2: body-parsing fallback', () => {
  ...
  it('Bokadirekt "Customer:" label', () => {
    const r = P.parse({
      senderEmail: 'no-reply@bokadirekt.se', senderName: 'Bokadirekt',
      subject: 'Booking',
  ...
  it('Generic "Patient:" label på okänt system', () => {
    const r = P.parse({
      senderEmail: 'no-reply@randomsystem.com', senderName: 'No Reply',
      subject: 'Update',
  ...
  it('Signatur "Med vänlig hälsning"', () => {
    const r = P.parse({
      senderEmail: 'no-reply@example.com', senderName: 'No Reply',
      subject: 'Foo',
      body: 'Bla bla.\nMed vänlig hälsning,\nErik Larsson',
    });
  ...
  it('nameFromEmail när allt annat misslyckas', () => {
    const r = P.parse({
      senderEmail: 'anna.karlsson@hotmail.com', senderName: 'noreply',
      subject: 'Foo', body: '',
    });
...
describe('Edge cases: tomma/null inputs', () => {
  ...
  it('Undefined fält → null', () => {
    eq(P.parse({ senderEmail: undefined, senderName: undefined, subject: undefined, body: undefined }), null);
  });
...
describe('Edge cases: falska positiva (ska INTE matcha)', () => {
  it('"Hej Anna" i body är TILL oss, inte FRÅN Anna', () => {
    const r = P.parse({
      senderEmail: 'no-reply@example.com', senderName: 'No Reply',
      subject: 'Foo', body: 'Hej Anna, kan ni boka tid?',
    });
    eq(r && r.customerName, null);
  });
  it('Marketing från Revolut — ingen kund', () => {
    const r = P.parse({
      senderEmail: 'noreply@revolut.com', senderName: 'Revolut',
      subject: 'Your weekly summary', body: 'Tap to see your spending',
    });
  ...
  it('HelloFresh marketing — ingen kund', () => {
    const r = P.parse({
      senderEmail: 'hello@mail.hellofresh.se', senderName: 'HelloFresh',
      subject: 'Veckans recept väntar', body: 'Klicka här för dina måltider',
    });
    if (r) eq(r.customerName, null);
  });
  it('Företagsnamn med stora bokstäver — Hair TP Clinic blockas', () => {
    const r = P.parse({
      senderEmail: 'no-reply@example.com', senderName: 'No Reply',
      subject: 'Hair TP Clinic — uppdatering', body: '',
    });
    eq(r && r.customerName, null);
  });
  it('Vanlig person-mejl (inte system) → null', () => {
    const r = P.parse({
      senderEmail: 'info@example.com', senderName: 'Info',
      subject: 'Hej', body: 'Tjena, ny fråga.',
    });
    eq(r, null);
  });
});
...
describe('Edge cases: namn-varianter', () => {
  it('Förnamn med bindestreck "Anne-Marie Dupont"', () => {
    const name = P.bodyHelpers.matchLabeledName(
      'Kund: Anne-Marie Dupont\n',
      ['kund'],
    );
  ...
  it('Trippel-namn "Maria del Carmen Lopez"', () => {
    const name = P.bodyHelpers.matchLabeledName(
      'Customer: Maria del Carmen Lopez\n',
      ['customer'],
    );
...
describe('Edge cases: HTML & citat-blocks', () => {
  ...
  it('Indenterad citat-rad (> Från:)', () => {
    const r = P.bodyHelpers.matchQuotedFrom(
      '> Från: Anders Petterson <x@y.se>\n> Skickat: 15 maj',
    );
...
describe('isSystemSender edge cases', () => {
  ...
  it('Bara senderName säger noreply', () => eq(P.isSystemSender('anna@example.com', 'Noreply'), true));
  it('Känd system-domän transactional@bokadirekt.se', () => eq(P.isSystemSender('transactional@bokadirekt.se', ''), true));
  it('Vanlig person-email/namn → false', () => eq(P.isSystemSender('anna@example.com', 'Anna'), false));
...
describe('looksLikePersonName edge cases', () => {
  ...
  it('Lång string (>60 chars) → null', () => {
    const r = P.bodyHelpers.matchLabeledName(
      'Kund: ' + 'A'.repeat(70) + '\n',
      ['kund'],
    );
    eq(r, null);
