...
test('PR25: workbench-flikarna får en egen full-breddsrad', () => {
  const block = konv.match(/\.action-modal--workbench \.action-modal-tabs\s*\{[^}]*\}/);
  assert.ok(block, 'workbench-tabs-override saknas');
  assert.match(block[0], /flex-basis:\s*100%/);
});
