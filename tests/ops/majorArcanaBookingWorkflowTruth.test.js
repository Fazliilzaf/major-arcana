...
test('telefonbokningsläget lyfter valt slot, extern bekräftelse och audit högre upp i bookingytan', () => {
  ...
  assertMatchFast(
    source,
    /<section class="booking-phone-workflow" data-booking-phone-workflow aria-label="Telefonbokning">[\s\S]*data-booking-phone-title[\s\S]*data-booking-phone-progress[\s\S]*data-booking-phone-slot[\s\S]*data-booking-phone-selected-audit[\s\S]*data-booking-phone-confirmed-audit[\s\S]*data-booking-phone-activity-title[\s\S]*data-booking-phone-activity-meta[\s\S]*data-booking-phone-wait-title[\s\S]*data-booking-phone-wait-meta[\s\S]*data-booking-phone-primary[\s\S]*data-booking-phone-secondary[\s\S]*data-booking-phone-confirmation-title[\s\S]*data-booking-phone-confirmation-copy[\s\S]*data-booking-phone-next-title[\s\S]*data-booking-phone-next-copy/,
    'Förväntade att bookingytan renderar ett eget telefonbokningskort nära toppen med stegstatus, slotstatus, extern bekräftelse, senaste bookinghändelse, kundvänteläge, guidance och audit sammanhållna.'
  );

  assertMatchFast(
    source,
    /const phoneWorkflow = getBookingPhoneWorkflowSummary\(readout\);[\s\S]*bookingDom\.phoneTitle[\s\S]*bookingDom\.phoneState[\s\S]*bookingDom\.phoneProgress[\s\S]*buildBookingPhoneWorkflowProgressMarkup\(phoneWorkflow\)[\s\S]*bookingDom\.phoneSlot[\s\S]*bookingDom\.phoneSelectedAudit[\s\S]*bookingDom\.phoneConfirmedAudit[\s\S]*const phoneActivity = getBookingPhoneLatestActivityReadout\(readout\);[\s\S]*bookingDom\.phoneActivityTitle[\s\S]*bookingDom\.phoneActivityMeta[\s\S]*const phoneWait = getBookingPhoneCustomerWaitReadout\(readout\);[\s\S]*bookingDom\.phoneWaitTitle[\s\S]*bookingDom\.phoneWaitMeta[\s\S]*bookingDom\.phoneConfirmationTitle[\s\S]*bookingDom\.phoneConfirmationCopy[\s\S]*bookingDom\.phoneNextTitle[\s\S]*bookingDom\.phoneNextCopy[\s\S]*bookingDom\.phonePrimary[\s\S]*bookingDom\.phoneSecondary/,
    'Förväntade att rendern faktiskt hydratiserar telefonbokningskortet från booking-readoutet, inklusive en tydlig progressionsrad, en kompakt senaste-händelse-yta och ett synligt kundvänteläge i stället för att lämna läget statiskt.'
  );
