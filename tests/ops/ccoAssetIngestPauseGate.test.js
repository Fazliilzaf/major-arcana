...
const {
  ...
  evaluateHardGate,
  shouldAutoResume,
} = require('../../src/ops/ccoAssetIngestPauseGate');

function freshRoot() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ingest-pause-'));
}
...
test('restart after normal process restart resumes when no pause file present', () => {
  const dir = freshRoot();
  assert.equal(readPause(dir), null);
  const decision = shouldAutoResume({ enabled: true, running: false, paused: false });
  assert.deepEqual(decision, { resume: true });
});

test('hard gate stops and does not restart', () => {
  const dir = freshRoot();
  writePause(dir, { reason: 'failed_imports' });
  const pause = readPause(dir);
  assert.ok(pause);
  assert.equal(pause.paused, true);
  assert.equal(pause.reason, 'failed_imports');
  const decision = shouldAutoResume({ enabled: true, running: false, paused: true });
  assert.deepEqual(decision, { resume: false, reason: 'persistent_pause' });
});

test('manual persistent pause survives a fresh process read', () => {
  const dir = freshRoot();
  writePause(dir, { reason: 'manual_stop' });
  // Simulate a brand-new process: read straight from disk again.
  const pause = readPause(dir);
  assert.ok(pause);
  assert.equal(pause.paused, true);
  assert.equal(pause.reason, 'manual_stop');
  const decision = shouldAutoResume({ enabled: true, running: false, paused: !!pause });
  assert.equal(decision.resume, false);
  assert.equal(decision.reason, 'persistent_pause');
});

test('resume starts again only after explicit resume', () => {
  const dir = freshRoot();
  writePause(dir, { reason: 'orphan_files' });
  assert.equal(shouldAutoResume({ enabled: true, running: false, paused: true }).resume, false);
  clearPause(dir);
  assert.equal(readPause(dir), null);
  const decision = shouldAutoResume({ enabled: true, running: false, paused: false });
