...
test('linkPatientToMessage creates audit event on relink', async () => {
  ...
  const audit = store
    .getState()
    .auditEvents.find((e) => e.type === 'mail_ingestion_patient_relinked');
...
test('linkPatientToMessage builds thread identity and flags conflict', async () => {
  ...
  const savedA = await store.saveRawMessageFromTruth({
    truthMessage: {
      ...baseTruth,
      graphMessageId: 'graph-msg-ti-1',
      internetMessageId: '<ti-1@example.com>',
    },
    mailAccountId: account.id,
    importRunId: run.id,
  });
  const savedB = await store.saveRawMessageFromTruth({
    truthMessage: {
      ...baseTruth,
      graphMessageId: 'graph-msg-ti-2',
      internetMessageId: '<ti-2@example.com>',
    },
    mailAccountId: account.id,
    importRunId: run.id,
  });

  await store.updateLedger(savedA.ledger.id, {
    status: 'UNMATCHED',
    patientMatchStatus: 'UNMATCHED',
  });
  await store.updateLedger(savedB.ledger.id, {
    status: 'UNMATCHED',
    patientMatchStatus: 'UNMATCHED',
  });

  await store.linkPatientToMessage({
    rawMessageId: savedA.rawMessage.id,
    patientId: 'patient-a',
    actorUserId: 'owner@test',
  });
  ...
  await store.linkPatientToMessage({
    rawMessageId: savedB.rawMessage.id,
    patientId: 'patient-b',
    actorUserId: 'owner@test',
  });
