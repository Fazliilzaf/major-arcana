...
| H5 | *(okänd ID)* | ENG \| Health Questionnaire | Patient | `health_declaration` | `eng` | Engelsk översättning av H1 | Konsultation | Patient | 2 | **Ny variant** |
| H6 | *(duplicate)* | Copy - Hälsodeklaration | — | — | — | — | — | — | — | **Migrera ej** — dedupe mot H1 |
...
| J5 | *(draft)* | FÖRSLAG \| Journal TP | 59 | `tp_treatment` | — | — | — | — | **Migrera ej** — använd J1 som canonical |

**J1 fältmapping (Meridiq 16411 → Arcana, urval):**

| Meridiq-sektion | Arcana-fält (`tp_treatment`) |
|-----------------|------------------------------|
| Metod FUE/DHI/kombination | `metodFue`, `metodDhi`, `metodKombination` *(legacy: `metod`)* |
| Områden vikar/krona/front/skägg/ögonbryn/ärr | `omradeVikar` … `omradeArr` *(legacy: `behandlingsomraden[]`)* |
...
| Vitalparametrar | `blodtryckMmHg`, `vitalKlockslag` *(legacy: `puls`)* |
| Observationer under ingrepp | `obsLangreHarligne` … `obsSegHud` *(legacy: `observationerUnderIngrepp[]`)* |
...
| U3 | **16390** | Journal \| TP Resultatuppföljning (12 månader) | 1 | `follow_up` | `12_manader` | `followup_12m` | 2 | ✅ schema-driven *(Meridiq-mall har 1 fält)* |
...
| **Curatiio ortopedi** | H3 | *(ev. separat)* | — | — | **1+** |
| **Curatiio estetik** | H4 | *(via samtycke)* | — | — | **1+** |
| **Engelska** | H5 | — | — | — | **1** |
...
```javascript
'consultation-online', 'consultation-physical', 'followup-transplant',
'fue', 'dhi', 'beard', 'eyebrow',
'prp-hair', 'prp-skin', 'microneedling', 'followup'
```
...
| Per-resurs SMS 4h/24h + ICS | Resend + SMS *(SMS saknas)* | P0 |
...
- [ ] **Bokning:** Alla Plan A-tjänster bokas i Arcana; Cliento widget avstängd
- [ ] **Formulär:** Alla 14 aktiva Meridiq-formulär (exkl. utkast/dubbletter) ifylls i Arcana med signering + PDF
- [ ] **Samtycken:** Behandlingsavtal per tjänstegrupp; distans betänketid enforced
- [ ] **Journal:** TP/PRP/uppföljning live för personal (mobil + desktop)
- [ ] **Kassa:** Minst kvitto + tjänstebetalning på encounter
- [ ] **Kommunikation:** Boknings-SMS/mejl + offer workflow
- [ ] **Migration:** Historik från Meridiq + Drive importerad; Meridiq read-only
- [ ] **Compliance:** Audit log, 10 år retention, EU-lagring, inga AI-tredjeparter i journal
...
*Detta dokument är canonical källa för formulär-migration och systemmerge. Uppdatera vid varje Meridiq/Cliento-export eller ny `journalType` i `ccoJournalStore.js`.*
