# Calendar Migration Matrix: kalender.html → kunder.html calendar-shell

*Genererad: 2026-05-30 · Status: AUDIT — väntar på owner-approval innan implementation*

> Owner-direktiv: aktivera existing `.calendar-shell` i `kunder.html`, flytta
...
### Finns redan i `kunder.html` (rad 3327–3645)
...
| # | Funktion i kalender.html | Finns redan i kunder.html? | Ska flyttas? | Ska ersättas? | Ska tas bort? | Risk om tappas | Testkrav |
|--:|---|---|---|---|---|---|---|
...
| 3 | **Booking-create-modal** | ❌ NEJ — finns inte | ✅ **flyttas** `openCreateBookingModal` + CSS | Helt ny i kunder.html | Nej | **HÖG** — kan inte boka | Smoke: 5 quick-picks, submit → POST 200 |
...
| 5 | **Quick-picks (5 service-typer)** | ❌ NEJ — kunder.html har bara segment-tabs för modes | ✅ **flyttas** quick-pick-grid | Helt ny | Nej | **MEDEL** — staff väljer manuellt service-id | Smoke: 5 picks renderas, click → state-update |
| 6 | **Booking-drawer** (right + mobile bottom-sheet) | ❌ NEJ | ✅ **flyttas** `renderDrawer` + CSS | Helt ny i kunder.html (app-grid har 360px höger-kolumn — kan återanvändas) | Nej | **HÖG** — kan inte se booking-detalj | Smoke: klick på booking → drawer öppen med 6 actions |
...
| 14 | **Öppna patientkort action** | 🟡 PARTIAL — kunder.html ÄR patientkort-vyn men ingen deep-link från booking-card | ✅ **flyttas** scroll-till-kund-handler | Ny: tab-switch till kund-vyn med highlight | Nej | **MEDEL** — staff söker manuellt | Smoke: klick → switch till kunder-tab + visa kort för patientId |

### Komplement (existing features i kunder.html som BEHÅLLS)
...
4. Inkludera i `kunder.html` head: `<script src="/cco-kalender-shell.js" defer>`
...
6. Drawer renderas i kunder.html `.right-col` (befintlig 360px-kolumn i `.app-grid`) när booking klickas
...
12. Lägg in `<meta http-equiv="refresh" content="0; url=/kunder.html?view=calendar">` i `kalender.html`
...
### Fas 1 (dagvy)
- [ ] `.calendar-shell` synlig på `/kunder.html?view=calendar`
...
- [ ] Alla 14 funktioner verifierade i kunder.html
- [ ] kalender.html redirectar till /kunder.html?view=calendar
...
|---|---|---|
| Brytning av existing kunder.html (291 KB fil) | MEDEL | Extra-kod i ny `cco-kalender-shell.js`, minimala in-file edits |
| Konflikt mellan existing JS-handlers (mic/calm/avatar) och nya | LÅG | Använda andra event-targets, scoped variabler |
| CSS-konflikt mellan `cco-kalender.css` och existing `kunder.html`-CSS | MEDEL | Prefix-rename `.cal-*` → `.cco-cal-*` om kollision |
...
4. **`cco-kalender.css` namnkollision?**
   - Risk: `.cal-*` kan kollidera med kunder.html-classes
...
| Status | Värde |
|---|--:|
| Existing i kunder.html | 13 komponenter (calendar-shell + toolbar + status-bar + morgon-story + 4 segment-tabs + density + mic + calm + avatar + timemachine) |
...
| In-place edits i kunder.html | ~5 rader (remove hidden + setMode + script-tag + 2 mount-points) |
...
*Refs:*
- *kunder.html L112-630 (calendar CSS), L3327-3645 (calendar HTML), L4081 (boot)*
