...
- [x] OWNER MFA secret/recovery sparade säkert
- [x] Underhållsfönster kommunicerat (~15 min)
...
- [x] STAFF kan logga in + nå journal + Ta bild på mobil (iPhone + Android manuellt)
- [x] OWNER login kräver MFA (enforced)
...
- [x] Graph send live i prod (secrets + beslut) — `npm run verify:graph-send-prod`
- [x] Playwright smoke `/uppfoljning/[token]` — `npm run verify:post-op-uppfoljning-prod` / `test:playwright:post-op`
...
- [x] Daglig rapport: saknade formulär/samtycken
- [x] Journalutkast med human approval (gateway)
- [x] Scheduler + OWNER notify
...
- [x] Phase 2 Workstream A säkerhet grön (MFA all roles, audit immutability, …)
- [x] Risk + policy gates verifierade under last
- [x] Patient guardrails + beta-gate (redan delvis byggt)
- [x] Formell GO/NO-GO: [arcana-master-plan-punktvis.md](./arcana-master-plan-punktvis.md) §9
