...
**Status:** Körning 2026-05-23  
**Branch:** `feat/abcde-workstreams`  
...
|----|---------|--------|---------|
| B1 | Prod deploy → catalog = 3 services | ⏸ | Render deploy efter merge |
| B2 | Prod curl verify | ⏸ | B1 |
...
| C4 | SharePoint archive sync | ⏸ | `bash scripts/sync-sharepoint-archive.sh` (lokal path) |
...
| D5 | Wire `GenerateContentSeries` i compose | ✅ | `node tests/_cmoMutationRunner.js` |
...
5. npm run migration:spot-check          # lokalt data/
6. npm run verify:cco-mail-start-prod    # prod
7. npm run verify:booking-plan-a-prod    # prod catalog
...
- [ ] B2 prod curl grön (efter deploy)
- [ ] B3/E2 Resend live
