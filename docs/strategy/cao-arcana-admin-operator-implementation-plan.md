...
Syfte: Utöka **CAO** från dagens *Template Advisor* till **Arcana Admin Operator** — agenten som håller ordning på admin, mallar, dokumentation, processer, kvalitet och intern kontroll.  
Källor:
...
**Idag:** CAO = `POST /api/v1/agents/CAO/run` med tre capabilities (SuggestTemplateImprovement, ValidateDisclaimers, OptimizeVariables). Admin-orchestratorn (`POST /api/v1/orchestrator/admin-run`) *planerar* adminåtgärder men exekverar dem inte. Obligatorisk pipeline finns i gateway.
...
**Produktnamn i UI:** *Arcana Admin Operator* (kodnamn kan fortfarande vara `CAO` i API/registry).
...
| **Admin Orchestrator** | Intent → action plan → *föreslagna* API-anrop | `adminOrchestrator.js` |
...
- [ ] Besluta produktnamn: UI “Arcana Admin Operator”, API `CAO` vs ny `CAO_OPERATOR`
- [ ] Skriv ADR: CAO scope expansion + orchestrator execute boundary
- [ ] Uppdatera `arcana-master-plan-punktvis.md` §8.2 med 5 “extra starka” områden (mallbibliotek, admin QC, incident/SLA, Go/No-Go-underlag, audit-dok)
- [ ] Riskklassificera alla 15 kapacitetsområden (L1–L5) i tabell
- [ ] Definiera output-typer: `AdminBrief`, `TemplateLibraryReport`, `QualityGateReport`, `GoNoGoBrief`

### Fas 1 — Mallbibliotek & admin quality gate (2–3 veckor)

**Mål:** CAO gör mer än disclaimer-scan; varje task/mall har owner, status, DoD.

- [ ] Capability: `AssessTemplateLibraryHealth` (version, inkonsistens, stale flags)
- [ ] Capability: `AssessAdminQualityGate` (tasks utan owner/status/dod från konfigurerad källa)
- [ ] Utöka `composeCaoTemplateAdvisor` → `composeCaoAdminOperator` med bakåtkompatibel output
- [ ] Uppdatera `registry.js` CAO bundle capability-lista
- [ ] `executionService.js`: snapshot hydration (templates + audit + open incidents)
- [ ] Admin UI: visa quality-gate-resultat + länk till mallar att fixa
- [ ] Tester: `tests/agents/caoAgentGateway.test.js` (spegla COO-test)
- [ ] `executiveDecisionFeed.js`: nya action types (`fix_template`, `assign_owner`)
...
- [ ] `adminOrchestrator.js`: `buildExecutableSteps()` — filtrera steg med `autoExecuteAllowed`
- [ ] Ny route eller flagga: `POST /api/v1/orchestrator/admin-run?mode=execute`
- [ ] Map intents → CAO capabilities (template_library → CAO run med intent payload)
- [ ] Audit: logga orchestrator plan + executed steps + correlationId
- [ ] Admin UI: “Kör rekommenderad admin-plan” med preview av steg

### Fas 3 — Dokumentation & process (2 veckor)

- [ ] Capability: `AuditDocumentationMetadata` (saknar ägare/datum/status/nästa steg) mot `docs/major-arcana-index.md` + strategi/ops-träd
- [ ] Capability: `ProposeDocumentStructure` (förslag, ingen auto-flytt av filer)
- [ ] Mallgenerator: onboarding, incident, SLA, DoD, möte, release notes, runbook (från spec §2) som **draft templates** i `data/templates.json`
- [ ] Scheduler-förslag: `scripts/` job som kör CAO quality gate dagligen (OWNER notify)

### Fas 4 — Incident, SLA & compliance admin (2–3 veckor)

- [ ] Capability: `SummarizeIncidentAdmin` (ägare, status, deadline, severity, SLA-risk)
- [ ] Capability: `FlagUnownedIncidents` + integration med COO output
- [ ] Capability: `BuildAuditSummary` (read-only från `authStore` audit events)
- [ ] Capability: `VerifyDecisionTraceability` (ändring utan motivering/godkännande → flag)
- [ ] Admin dashboard: incident/SLA-vy (§13)
- [ ] Runbook: uppdatera `docs/ops/runbooks/patient-safety-incident-runbook.md` med CAO admin-steg

### Fas 5 — Tenant, onboarding & rapportering (2 veckor)

- [ ] Capability: `TenantAdminHealthSummary` (status, owner, policy profile, isolation checklist)
- [ ] Capability: `GenerateAdminDailyBrief` / `WeeklyAdminBrief` (komponera COO + CAO + audit)
- [ ] Capability: `ExplainReadinessScore` (koppla till monitor Go/No-Go §9)
- [ ] Rollbaserade checklistor (OWNER vs team) som template-sets
- [ ] CFO/CMO/CCO-sammanfattningar: separata compose-warnings, ingen data-leak mellan agenter

### Fas 6 — Automation & adminpanel-struktur (2 veckor)

- [ ] Identifiera scheduler-jobb (daglig adminrapport, SLA-risk scan, saknad DoD)
- [ ] Notifieringsregler (notify efter gateway, OWNER-only för L4+)
- [ ] Adminpanel informationsarkitektur (§13): task-, incident-, mall-, dokument-, audit-vyer
- [ ] Dokumentera API i `docs/ops/developer-handover.md`

### Fas 7 — Hardening & Go/No-Go (1–2 veckor)

- [ ] E2E: admin-run plan + execute med mock data
- [ ] Stryker/contract tests på nya capabilities
- [ ] Readiness: CAO får inte sänka score (monitor checks)
- [ ] Go/No-Go-underlag genereras av CAO men **beslut** förblir OWNER + COO
