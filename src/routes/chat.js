...
function buildHairTPFallbackReply({ brand, message }) {
  ...
  if (
    hasAny([
      ...
      /allt ingår/,
      /allt ingar/,
      /dolda (kostnader|avgifter|tillägg|tillagg)/,
      /tillägg/,
      /tillagg/,
    ])
  ) {
    return joinLines([
      'I fastpriset för hårtransplantation ingår alltid:',
      '- Kostnadsfri konsultation före behandling',
      '- Bedövning och nödvändiga mediciner',
      '- Fyra PRP-behandlingar efter transplantationen',
      '- Tre uppföljningar under första året',
      '- Skriftlig prognos och journal',
      'Inga dolda tillägg.',
    ]);
  }

  // Hur lang tid tar behandlingen
  if (
    hasAny([
      /hur lang tid/,
      /hur lång tid/,
      /tar (det|behandlingen)/,
      /hur lange tar/,
      /hur länge tar/,
      /timmar tar/,
      /lang process/,
    ...
    return joinLines([
      'En hårtransplantation tar 4-8 timmar beroende på antal grafter.',
      'Du går hem samma dag.',
    ]);
  }
  ...
  if (
    hasAny([
      ...
      /när ser jag/,
      /resultat/,
      /hur lang tid.*resultat/,
      /hur lång tid.*resultat/,
      /sluttillvaxt/,
      /sluttillväxt/,
      /slutresultat/,
    ])
    ...
    return joinLines([
      'Tydlig skillnad syns efter cirka 6 månader.',
      'Slutresultatet ses efter 10-12 månader, då kollagenbildning och full hårtillväxt är klar.',
    ]);
  }
  ...
  if (
    ...
    return joinLines([
      'Vi använder lokalbedövning under behandlingen.',
      'De flesta upplever endast lätt obehag, jämförbart med ett myggbett.',
      'Eftervården kräver vanligen bara paracetamol.',
    ]);
  }

  // Permanent / haller / hur lange varar
  if (
    hasAny([
      /permanent/,
      /haller (det|hartransplantation)/,
      /håller (det|hårtransplantation)/,
      /livslang/,
      /livslång/,
      /hela livet/,
      /haller hela/,
    ...
    return joinLines([
      ...
      'De transplanterade hårsäckarna kommer från donatorområdet bak i huvudet och är genetiskt resistenta mot DHT-hormonet.',
      'De fortsätter därför växa hela livet.',
    ]);
  }

  // Avbetalning / finansiering / delbetala
  if (
    hasAny([
      /avbetalning/,
      /delbetala/,
