 ...
 *   - Varje endpoint kräver requireAuth + kanonisk finansauktorisation
 *     (owner/finance/revisor via FINANCE_ROLES) + tenantId
...
function createReconciliationRouter({
  ...
  function buildMergeFns({ tenantId, actor }) {
    return async function transferFn({ primaryPatientId, secondaryPatientId }) {
      ...
      const filesAfter =
        toPnr && migrationIndexStore?.getFilesForPersonnummer
  ...
  function buildRollbackFn({ tenantId, actor }) {
    return async function rollbackFn({ primaryPatientId, secondaryPatientId }) {
      ...
      if (secondary && secondary.matchStatus === 'merged' && patientMasterStore?.upsertPatient) {
    ...
    async (req, res, next) => {
      try {
        ...
        const results = patients.map((p) =>
          computePatientCompleteness(p, driveFiles, journalEntries)
        );
  ...
  router.get(
    '/reconciliation/completeness/:patientId',
    requireAuth,
    requireAnyRole(FINANCE_ROLES),
    async (req, res, next) => {
      try {
  ...
  router.get(
    '/reconciliation/uncertain-files',
    requireAuth,
    requireAnyRole(FINANCE_ROLES),
    (req, res) => {
      const driveFiles = loadDriveFiles();
  ...
  router.post(
    '/reconciliation/merge/:mergeId/approve',
    requireAuth,
    requireAnyRole(FINANCE_ROLES),
    async (req, res, next) => {
      try {
  ...
  router.post(
    '/reconciliation/merge/:mergeId/execute',
    requireAuth,
    requireAnyRole(FINANCE_ROLES),
    async (req, res, next) => {
      try {
        ...
        const result = await mergeService.executeMerge(req.params.mergeId, {
          transferFn,
          rollbackFn,
        });
  ...
  router.post(
    '/reconciliation/merge/:mergeId/verify',
    requireAuth,
    requireAnyRole(FINANCE_ROLES),
    async (req, res, next) => {
      try {
        ...
        const filesActual =
          personnummer && migrationIndexStore?.getFilesForPersonnummer
        ...
        if (
          secondaryPnr &&
          secondaryPnr !== personnummer &&
          migrationIndexStore?.getFilesForPersonnummer
        ) {
  ...
  router.post(
    '/reconciliation/merge/:mergeId/rollback',
    requireAuth,
    requireAnyRole(FINANCE_ROLES),
    async (req, res, next) => {
      try {
  ...
  router.get('/reconciliation/merges', requireAuth, requireAnyRole(FINANCE_ROLES), (req, res) => {
