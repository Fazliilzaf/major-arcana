'use strict';
const { substitute } = require('../ops/ccoMessageRenderer');
...
function buildDraftFromConversation(conversation, { tone = 'professional', templates = [] } = {}) {
  const customerName = normalizeText(
    conversation?.customerName || conversation?.from?.name || 'kund'
  );
  ...
  if (matchingTemplate?.content) {
    // ORD-111: samma renderare som registret (camelCase-konvention). Alias för
    // gamla snake_case-mallar behålls bakåtkompatibelt tills de flyttas till
    // firstName/clinicName. En ofylld variabel kastas (se ccoMessageRenderer).
    body = substitute(matchingTemplate.content, {
      firstName: customerName,
      first_name: customerName,
      clinicName: 'Hair TP Clinic',
      clinic_name: 'Hair TP Clinic',
    });
  ...
  return {
    ...
    templateUsed: matchingTemplate
      ? normalizeText(matchingTemplate?.name || matchingTemplate?.id)
      : null,
...
class PrepareResponseDraftsCapability extends BaseCapability {
  ...
  static outputSchema = {
    ...
    properties: {
      data: {
        type: 'object',
        required: ['drafts', 'summary', 'generatedAt'],
  ...
  async execute(context = {}) {
    ...
    const tone = TONE_VARIANTS.includes(normalizeText(input.tone))
    ...
    if (needsReply.length === 0 && conversations.length > 0) {
      warnings.push(
        'Inga konversationer markerade som needs_reply hittades. Visar drafts för senaste inbound.'
      );
      needsReply.push(
        ...conversations
    ...
    const drafts = needsReply
      .slice(0, maxDrafts)
      .map((conversation) => buildDraftFromConversation(conversation, { tone, templates }));

    const highConfidence = drafts.filter((d) => d.confidence === 'high').length;
    const summary =
      `${drafts.length} svarsutkast förberedda (ton: ${tone}). ` +
