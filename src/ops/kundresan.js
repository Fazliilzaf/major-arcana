 ...
 *
 * FILEN KÖRS PÅ BÅDA SIDOR. Servern kräver in den; webbläsaren laddar exakt
 * samma fil via /kundresan.js. Det är avsiktligt och det är hela poängen: en
 * portad kopia hade blivit en femte uträkning som liknar de andra tills den
 * inte gör det längre.
 *
 * Facit skickas in i stället för att kräva:s in, eftersom webbläsaren inte kan
 * require:a JSON. Servern skickar filen, klienten hämtar samma fil över nätet.
 *
...
(function (root, fabrik) {
  // Node: facit kräver:s in här. Webbläsare: skickas in via skapaKundresa().
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = fabrik(require('../../config/kundresan-13-steg.json'));
    module.exports.skapaKundresa = fabrik;
    return;
  }
  root.ArcanaKundresa = { skapaKundresa: fabrik };
})(typeof globalThis !== 'undefined' ? globalThis : this, function (FACIT) {
  'use strict';

  const STEG = Array.isArray(FACIT && FACIT.steg) ? FACIT.steg : [];
  const VARIANTER = (FACIT && FACIT.varianter) || {};
  ...
  return {
    beraknaKundresa: beraknaKundresa,
    valjVariant: valjVariant,
    STEG: STEG,
    STATUS: STATUS,
